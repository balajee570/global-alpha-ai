...
def _recommend_action_impl(metrics: dict, tech: dict) -> dict:
    ...
    if stage_id == 4:                 action = "AVOID"
    elif conviction >= 80 and not critical_bear: action = "STRONG_BUY"
    elif conviction >= 65:            action = "BUY"
    elif conviction >= 50:            action = "ACCUMULATE"
    elif conviction >= 35:            action = "HOLD"
    else:                             action = "AVOID"
    ...
    with tab_scan:
        if not momentum_df.empty:
            ...
            display_cols = [c for c in [
                "Ticker","Name","Sector","Industry","Country","Price $","Grade","Stage","Score","RS Rank","Pattern","Pattern Q",
                "Signal","RSI","Vol Ratio","ADX","MACD Hist","RS vs NDX","ROC20 %",
