...
def get_global_stocks() -> pd.DataFrame:
    """
    Fetch all US stocks from SEC company tickers JSON
    """
    try:
        headers = {
        ...
        for key, value in data.items():
            ticker = value.get("ticker", "").strip().upper()
            name = value.get("title", "").strip()
            
            # Skip if no ticker
            if not ticker:
                continue
            
            # Skip ETFs, Funds, Trusts (basic filtering)
            skip_keywords = ["ETF", "FUND", "TRUST", "INDEX", "NOTES"]
        ...
        if stocks:
            df = pd.DataFrame(stocks).drop_duplicates("SYMBOL").reset_index(drop=True)
            return df
            
    except Exception as e:
        st.warning(f"SEC API failed: {e}. Using fallback list.")
    
    # Fallback to major stocks if SEC fails
    fallback_stocks = [
        "AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "TSLA", "AVGO", "ORCL", "ADBE",
        "CRM", "AMD", "INTC", "QCOM", "TXN", "IBM", "NOW", "PANW", "SNOW", "UBER",
        "PYPL", "SHOP", "CRWD", "DDOG", "PLTR", "NET", "ACN", "JPM", "V", "MA",
        "BAC", "WFC", "GS", "MS", "BLK", "AXP", "LLY", "JNJ", "UNH", "ABBV",
        "MRK", "TMO", "ABT", "DHR", "PFE", "HD", "MCD", "NKE", "WMT", "COST",
        "XOM", "CVX", "COP", "GE", "CAT", "NFLX", "DIS", "VZ", "T", "BA",
        "NEE", "DUK", "AMT", "LIN", "SHW", "KO", "PEP", "PG", "CVS", "CI"
    ]
    
    stocks = [{"SYMBOL": s, "NAME": s, "EXCHANGE": "US", "YF_SYMBOL": s} for s in fallback_stocks]
    return pd.DataFrame(stocks)

# Global Sector Mapping (extended)
SECTOR_MAP = {
    ...
    "ACN": "Technology", "HPQ": "Technology", "DELL": "Technology", "CSCO": "Technology",
    "INTU": "Technology", "ATVI": "Technology", "EA": "Technology", "TTWO": "Technology",
    ...
    "ABNB": "Consumer Cyclical", "LULU": "Consumer Cyclical", "DE": "Consumer Cyclical",
    "NFLX": "Communication Services", "DIS": "Communication Services", 
    "VZ": "Communication Services", "T": "Communication Services",
    "CMCSA": "Communication Services", "TMUS": "Communication Services",
    "CHTR": "Communication Services", "SPOT": "Communication Services",
    ...
    "ADP": "Industrials", "WM": "Industrials", "CSX": "Industrials", "UNP": "Industrials",
    "FDX": "Industrials", "NSC": "Industrials", "ITW": "Industrials", "MMM": "Industrials",
    "WMT": "Consumer Defensive", "COST": "Consumer Defensive", "PG": "Consumer Defensive",
    "KO": "Consumer Defensive", "PEP": "Consumer Defensive", "DG": "Consumer Defensive",
    "DLTR": "Consumer Defensive", "MDLZ": "Consumer Defensive", "GIS": "Consumer Defensive",
    "XOM": "Energy", "CVX": "Energy", "COP": "Energy", "EOG": "Energy",
    "SLB": "Energy", "OXY": "Energy", "MPC": "Energy", "VLO": "Energy",
    "NEE": "Utilities", "DUK": "Utilities", "SO": "Utilities", "D": "Utilities",
    "AEP": "Utilities", "EXC": "Utilities", "SRE": "Utilities", "XEL": "Utilities",
    "AMT": "Real Estate", "PLD": "Real Estate", "CCI": "Real Estate", "EQIX": "Real Estate",
    "PSA": "Real Estate", "O": "Real Estate", "DLR": "Real Estate", "AVB": "Real Estate",
...
def scan_global_market(symbols: list, _counter=None, _pbar=None) -> pd.DataFrame:
    """
    Scan stocks for momentum signals - Breakout or Volume Building
    Same logic as NSE Alpha AI
    """
    results = []
    failed = []
    total = len(symbols)
    ...
    for i in range(0, total, CHUNK_SIZE):
        ...
        for ticker in tickers:
            try:
                ...
                # RELAXED: Only skip if price is extremely low (< $0.10)
                if price <= 0.10:
                    continue
                ...
                # MOMENTUM SIGNAL LOGIC (Same as NSE Alpha AI)
                is_breakout = price >= high20 * BREAKOUT_TOLERANCE
                is_vol_surge = vol_ratio > VOL_SURGE_THRESH
                
                # Stock must be above MA50 AND (breaking out OR building volume)
                if price > ma50 and (is_breakout or is_vol_surge):
                    signal_type = "Breakout" if is_breakout else "Building"
    ...
    # Sort: Breakouts first, then by volume ratio
    return (
...
with col_note:
    st.markdown(
        ...
        "SEC All Stocks  · AI news + strategy  · ~10-15 min</span>",
        unsafe_allow_html=True,
    )
...
if run and secrets_ok:
    ...
    _eq_status = st.empty()
    _eq_status.markdown("Loading stock registry from SEC...", unsafe_allow_html=True)
    ...
    st.info(f"Loaded {n_eq_total:,} stocks from SEC. Scanning for momentum signals...")
    ...
    if not momentum_df.empty:
        ...
        st.markdown(
            ...
            f"<div>Breakouts: <strong style='color: #00c896;'>{breakouts}</strong></div>"
            f"<div>Building: <strong style='color: #f59e0b;'>{building}</strong></div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            f"<div style='padding: 1rem; background: #1a1a2e; border-radius: 8px; margin: 1rem 0;'>"
            f"<div>Total Stocks: <strong>{n_eq_total:,}</strong></div>"
            f"<div>No momentum signals found</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
if "strategy" in st.session_state:
    ...
    <div style="text-align: center; padding: 4rem 2rem; color: #666;">
        ...
        <p style="font-size: 0.85rem; color: #555;">
            SEC All Stocks (Breakout + Building)  · Live prices  · AI strategy
        </p>
    </div>
    """, unsafe_allow_html=True)
