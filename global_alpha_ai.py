...
def get_tavily_client():
    try:
        ...
        st.error("TAVILY_API_KEY not found in secrets. Please add it to your Streamlit Cloud secrets.")
        return None
...
def get_global_stocks() -> pd.DataFrame:
    """
    Fetch global stocks from Nasdaq.
    For Streamlit Cloud deployment - fetches liquid stocks (> $300M market cap).
    """
    headers = {
    ...
    for ex_name, ex_code in exchanges:
        try:
            params = {
                "tableonly": "true",
                "limit": 5000,
            ...
            r = requests.get(NASDAQ_SCREENER_URL, headers=headers, params=params, timeout=30)
            r.raise_for_status()
            data = r.json()
            ...
            if "data" in data and "table" in data["data"] and "rows" in data["data"]["table"]:
                ...
                for row in rows:
                    ...
                    name = str(row.get("name", row.get("companyName", symbol))).strip()
                    
                    # Skip ETFs, funds, and trusts
                    skip_keywords = ["ETF", "FUND", "TRUST", "INDEX", "NOTES", "DEPOSIT"]
                    if any(kw in name.upper() for kw in skip_keywords):
                        continue
                    if "^" in symbol or "." in symbol:
                        continue
                    
                    # Only keep stocks with reasonable market cap (>$300M)
                    market_cap = row.get("marketCap", 0)
                    if market_cap and float(market_cap) < 300_000_000:
                        continue
    ...
    if all_stocks:
        df = pd.DataFrame(all_stocks).drop_duplicates("SYMBOL").reset_index(drop=True)
        return df
...
with col_note:
    st.markdown(
        ...
        "Global Stocks (>$300M Market Cap)  · AI news + strategy  · ~5-7 min</span>",
        unsafe_allow_html=True,
    )
...
if run and secrets_ok:
    ...
    _eq_status = st.empty()
    _eq_status.markdown("Loading global stock registry ...", unsafe_allow_html=True)
    
    registry = get_global_stocks()
    
    if registry.empty:
        st.error("Could not load stock registry. Please check your internet connection.")
        st.stop()
    ...
    st.markdown(
        ...
        f"<div>Global Equities: <strong>{n_eq_total:,}</strong></div>"
    ...
    _ai("Fetching live global market prices ...", 10)
    live_snapshot = get_live_global_snapshot()
    
    _ai("Pulling latest global news ...", 25)
    raw_news = get_raw_global_news()
    
    _ai("Analysing global market intelligence ...", 40)
    intel_summary = generate_global_intel_summary(raw_news, live_snapshot)
    
    _ai("Building global strategy brief ...", 70)
...
if "strategy" in st.session_state:
    ...
    <div style="text-align: center; padding: 4rem 2rem; color: #666;">
        ...
        <p style="font-size: 0.85rem; color: #555;">
            Global Stocks (>$300M Market Cap)  · Live prices  · AI strategy
        </p>
    </div>
    """, unsafe_allow_html=True)
